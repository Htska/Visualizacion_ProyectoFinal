...
void Gradient::CreateTriangleList(){
...
void Gradient::Vertex::InitVertex(int x, int y, int z){
    Pos=glm::vec3(x,y,z);
}

void Gradient::InitVertices(std::vector<Vertex>& Vertices){
    int Index{0};
...
void Gradient::InitIndices(std::vector<uint>& Indices){
...
void Gradient::initGeometry(){
    ...
    m_model_mat = glm::scale(m_model_mat, glm::vec3(0.022f)); // SOLO una vez
}
...
void Gradient::renderModel(const glm::mat4& view, const glm::mat4& projection, GLenum mode){
    ...
    glDrawElements(mode, (m_depth-1)*(m_width-1)*6, GL_UNSIGNED_INT, NULL);
